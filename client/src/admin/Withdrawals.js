...
export default function Withdrawals() {
  ...
  const fetchWithdrawals = async () => {
    setLoading(true);
    try {
      const res = await axios.get('https://qxtrand.onrender.com/api/api/admin/withdrawals');
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      if (action === 'approve') {
        await axios.post('https://qxtrand.onrender.com/api/api/admin/approve-withdrawal', { id });
      } else {
        const adminNote = prompt('Enter rejection note (optional):') || '';
        await axios.post('https://qxtrand.onrender.com/api/api/admin/reject-withdrawal', { id, adminNote });
      }
      fetchWithdrawals();
    } catch (err) {
      alert('Action failed');
