...
const Team = () => {
  ...
  const fetchOverview = () => {
    if (!user?._id) return;
    axios.get(`https://qxtrand.onrender.com/api/api/user/referral-overview/${user._id}`)
      .then(res => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      axios.get(`https://qxtrand.onrender.com/api/api/user/referral-overview/${user._id}`).then(res => {
