...
export default function USDTWithdrawalPage() {
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      const res = await axios.post('https://qxtrand.onrender.com/api/user/withdrawal-otp/send-otp', {
        email: user.email
      });
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const user = JSON.parse(localStorage.getItem('user'));
      const verifyRes = await axios.post('https://qxtrand.onrender.com/api/user/withdrawal-otp/verify-otp', {
        email: user.email,
        otp
      });
